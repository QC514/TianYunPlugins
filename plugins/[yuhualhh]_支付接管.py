...
# [author: yuhualhh]
# [service: 2550306191]
# [description: <br>❶该插件可为任意订阅源的Python、NodeJS插件提供支付接管与积分卡密系统服务。使内置微信支付被支付宝账单免挂或易支付或积分抵扣所接管，同时兼容V1接口(MD5签名方式)各大码支付平台。<br>❷先发指令『替换中间件』待提示替换成功，继续发送指令『开启支付接管』待提示已开启，然后重启奥特曼使插件生效，并前往插件配参添加需启用支付接管的插件。如需暂时关闭则发指令『关闭支付接管』<br>❸使用本插件需授予一定权限，前往"系统管理-插件权限"全部启用<br>❹更新日志: 2026.04.10 00:45 将羽化核心拆分成支付接管与文本转图两个单独的插件，需发指令『替换中间件』更新一下<img src="https://gcore.jsdelivr.net/gh/lhz03/img@29949cd67168912e9b439d29d52a1a509fc70be2/2026/03/28/72bbaadb24ca85e3afb59b270e448cdd.png">]
...
# [param: {"spliter":true}]
#[param: {"required":false,"key":"yuhua_epay.alipay_bill","bool":true,"placeholder":"","name":"某宝账单免挂","desc":"启用支付宝账单免挂通道收款，配置教程: https://my.feishu.cn/docx/R280dtOaWothSix6KBRcGFxQncb"}]
#[param: {"required":false,"key":"yuhua_epay.alipay_qrcode","placeholder":"","name":"支付宝收款码","desc":"支付宝账单免挂通道的收款码直链"}]
...
请在15秒内完成
回复"q"取消""")
        amount_input = target_sender.input(15000, 0, False)
...
回复"q"退出"""
        ...
        choice = target_sender.input(15000, 0, False)
