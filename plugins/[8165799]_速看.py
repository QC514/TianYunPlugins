...
# [description: 速看小说代挂提交插件，支持抓包完整URL整段提交。<br>v3.4修正清理bug：<br>1. 严格执行整段提交：用户发送的完整URL直接存入青龙，不进行任何参数分割或重组。<br>2. 修复因缺失签名参数导致的脚本运行失败问题。<br> 3.购插件送脚本，脚本在售后群1003974618。<br>]
...
# [param: {"required":true,"key":"dd_sk.dd_sk_osname","bool":false,"placeholder":"必填项,例:SUKAN_URL","name":"提交到系统的变量名","desc":"系统容器内速看的变量名(默认为SUKAN_URL)"}]
...
def getusercontent():
    ...
    dd_sk_osname = middleware.bucketGet('dd_sk', 'dd_sk_osname') or 'SUKAN_URL'
...
def nn_session_ids(input_str):
    ...
    try:
        ...
        if info:
            ...
            return submission_str, info['user_id'], info['user_id'], info['nickname']
        else:
...
==================""")
        ...
        try:
            # 校验并提取数据
            submission_str, device_id, user_id, nickname = nn_session_ids(instr)
            
            # 执行绑定
            if submission_str and user_id:
                process_account_binding(submission_str, device_id, user_id, nickname, remark)
...
def process_account_binding(submission_str, device_id, user_id, nickname, remark=""):
    """处理账号绑定逻辑"""
    account_key = str(user_id) # 速看直接用zyeid作为唯一标识
...
=================="""
        ...
        logger.info(f"用户 {userid} 绑定账号成功: {user_id}, 备注: {remark}")
