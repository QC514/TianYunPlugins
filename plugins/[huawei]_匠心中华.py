# [rule: ^匠心登录$|^登录匠心$|^匠心管理$|^管理匠心$|^匠心查询$|^查询匠心$|^匠心物流$|^物流匠心$|^匠心兑换$|^兑换匠心$|^匠心批量兑换$|^匠心批量地址$|^匠心授权$|^匠心教程$|^匠心清理$|^匠心上传$|^匠心地址$]
# [disable:false]
...
# [author: huawei]
# [service: 1603960061]
# [description: 介绍：匠心插件，支持扫码登录、查询、兑换、批量兑换、物流、地址管理与管理<br>指令：匠心登录、匠心管理、匠心查询、匠心兑换、匠心批量兑换、匠心物流、匠心地址、匠心批量地址、匠心授权、匠心清理、匠心教程、匠心上传<br>更新日志：1.3.5 新增「匠心批量兑换」「匠心批量地址」独立指令，支持多账号选择；兑换支持关键词搜索和翻页]
...
BUCKET_NICK = 'G_JXZH_nick'
BUCKET_POINTS = 'G_JXZH_points'
...
def save_nickname(phone: str, nickname: str):
    if nickname and is_usable_display_name(nickname):
        middleware.bucketSet(BUCKET_NICK, phone, nickname)


def get_nickname(phone: str) -> str:
    return str(middleware.bucketGet(BUCKET_NICK, phone) or '').strip()


def save_points_cache(phone: str, points: str):
    middleware.bucketSet(BUCKET_POINTS, phone, str(points or '0'))


def get_points_cache(phone: str) -> str:
    return str(middleware.bucketGet(BUCKET_POINTS, phone) or '').strip()
...
def del_account(phone: str, user_id=None, panel_config: str = '', panel_envname: str = ''):
    ...
    middleware.bucketDel(BUCKET_NICK, phone)
...
def build_signed_payload(extra=None, use_app_style: bool = False, sign_mode: str = 'encoded'):
    ...
    if sign_mode == 'raw':
        ...
        sign_source = urllib.parse.quote(joined, safe='-_.')
    ...
    try:
        ...
        if code == 1:
            ...
            if isinstance(raw_data, list):
                data = raw_data
            elif isinstance(raw_data, dict) and isinstance(raw_data.get('list'), list):
                data = raw_data['list']
            else:
                data = []
            return {
...
def exchange_token(wx_code):
    wx_info = wx_exchange_code_for_token(wx_code)
    if not wx_info:
        sender.reply('❌ 微信授权失败，无法获取用户身份')
        return None
    openid = wx_info.get('openid', '')
    unionid = wx_info.get('unionid', '')
    wx_access_token = wx_info.get('access_token', '')
    if not openid:
        sender.reply('❌ 未获取到微信 openid')
        return None
    nickname = ''
    picurl = ''
    if wx_access_token and openid:
        user_info = wx_get_userinfo(wx_access_token, openid)
        if user_info:
            nickname = user_info.get('nickname', '')
            picurl = user_info.get('picurl', '')
    try:
        result = signed_post_with_fallback(
            THIRD_LOGIN_URL,
            headers=get_app_base_headers(),
        ...
        if result.get('code') == 1 and result.get('data'):
            ...
            return {
                ...
                'unionid': unionid,
                'nickname': nickname,
                'picurl': data.get('picurl') or picurl,
                'raw': data,
            }
        print(f"换取token失败: {result.get('message')}")
        return None
    except Exception as e:
        print(f"换取token异常: {e}")
        return None
    ...
    try:
        ...
        while retry > 0:
            ...
            if code == 0:
                ...
                best_nick = username if is_usable_display_name(username) else decode_wechat_nickname(wx_nickname or '')
                save_nickname(account, best_nick)
                login_userinfo = get_task_userinfo(token)
                if login_userinfo:
                    save_points_cache(account, login_userinfo.get('points', '0'))
                display_mobile = mask_phone(account) if len(account) >= 11 else account
                display_name = best_nick if is_usable_display_name(best_nick) else display_mobile
...
def jxzh_query_detail_lines(account, today_time):
    ...
    if token:
        ...
        if userinfo:
            ...
            username = decode_wechat_nickname(userinfo.get('username', ''))
            save_points_cache(account, total_points)
...
def jxzh_logistics_detail_lines(account):
    ...
    all_orders = []
    for query_status in ('2', '3'):
        order_result = get_order_list(token, status=query_status, page=1, keywords='')
        if order_result.get('ok'):
            all_orders.extend(order_result.get('data') or [])
        elif query_status == '3' and not all_orders:
            lines.append(f"❌ 物流查询失败：{order_result.get('message') or '未知错误'}")
            return lines

    if not all_orders:
        lines.append("📦 暂无订单（待发货/待收货）")
        return lines

    lines.append(f"📦 订单：{len(all_orders)}个")

    for index, order in enumerate(all_orders[:LOGISTICS_ORDER_LIMIT], 1):
    ...
    if len(all_orders) > LOGISTICS_ORDER_LIMIT:
        lines.append(f"ℹ️ 仅展示前 {LOGISTICS_ORDER_LIMIT} 条订单")
    return lines
...
def _show_accounts_selection(accounts: list, title: str) -> str:
    sender.reply(f"⏳ 正在加载 {len(accounts)} 个账号积分...")
    for account in accounts:
        token = jxzh_token_value_for_account(account)
        if token:
            userinfo = get_task_userinfo(token)
            if userinfo:
                save_points_cache(account, userinfo.get('points', '0'))

    lines = [f"====={title}====="]
    for i, account in enumerate(accounts, 1):
        login_name = get_account_display_name(account)
        pts = get_points_cache(account)
        pts_text = f"⭐{pts}" if pts else '⭐-'
        has_token = '' if jxzh_token_value_for_account(account) else ' ❌无Token'
        lines.append(f"[{i}] {login_name}（{pts_text}）{has_token}")
    lines.append("-------------------")
    lines.append("💡 操作说明:")
    lines.append("• 支持单选: 1")
